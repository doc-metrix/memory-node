...
(function() {
	...
	*/
	...
	var NAMES = Object.keys( METRICS ),
		...
		NAMESLOWER = NAMES.slice(),
		DEVICENAMESLOWER = DEVICENAMES.slice();

	for ( i = 0; i < NAMESLOWER.length; i++ ) {
		NAMESLOWER[i] = NAMESLOWER[i].toLowerCase();
	}

	for ( i = 0; i < DEVICENAMESLOWER.length; i++ ) {
		DEVICENAMESLOWER[i] = DEVICENAMESLOWER[i].toLowerCase();
	}
	...
	*/
	Metrics.prototype.exists = function( name ) {
		...
		if ( NAMESLOWER.indexOf( name.toLowerCase() ) === -1 ) {
			return false;
		}
		return true;
	}; // end METHOD exists()
	...
	*/
	...
	/**
	* METHOD: getDevice( name )
	*	Returns a list of metric specifications associated with the device. If a device does not have any associated metric specifications, returns null.
	*
	* @param {String} name - device name
	* @returns {Object|null} list of metric specifications or null
	*/
	Metrics.prototype.getDevice = function( name ) {
		var indexOfName;
		if ( typeof name !== 'string' ) {
			throw new TypeError( 'getDevice()::invalid input argument. Device `name` must be a string.' );
		}
		indexOfName = DEVICENAMESLOWER.indexOf( name.toLowerCase() );
		if ( indexOfName === -1 ) {
			return null;
		}
		return JSON.parse( JSON.stringify( DEVICES[ DEVICENAMES[ indexOfName ] ] ) );
	}; // end METHOD getDevice()
